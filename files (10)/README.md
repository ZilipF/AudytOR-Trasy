...
nazwisko i wybierz **bazę wyjazdu** (Warszawa / Gdańsk / Nowa Wieś Wielka).
Każdy audytor planuje własną, osobną trasę.

Stacje trafiają do najbliższej bazy spośród tych, z których ktoś faktycznie
wyjeżdża — audytor z Gdańska nie dostanie stacji spod Krakowa. Jeśli z jednej
...
> będzie gotowa. Każdy audytor planuje osobno.

**2b. Wybierz tryb parametrów budowy tras.** Pod polem „Wlicz powrót do bazy"
są dwa tryby:

- **Tryb prosty** (domyślny) — parametry zaszyte na stałe: audyt 120 min,
...
- **Tryb rozszerzony** — odsłania te same wartości jako edytowalne pola:
  czas audytu stacji, warianty długości delegacji do wygenerowania (można
  odznaczyć np. wariant 3-dniowy, żeby przyspieszyć liczenie), budżet dnia,
  twardy limit dnia, przerwę i próg km dla delegacji wielodniowych. Przycisk
...
> Tryb i wartości są ustawieniem roboczym sesji — jak liczba audytorów czy
> baza wyjazdu, **nie zapisują się** w projekcie ani w przeglądarce. Po
...
karta dla każdego audytora**, a nie jeden wspólny wariant na wszystkich —
...
**5. Odhacz kontrole i wyślij audytorowi.** Przycisk ✓ przy stacji oznacza ją
...
dokładaj". Oznaczenie przeżywa usunięcie stacji z trasy czy skasowanie całej
trasy (to trwała wiedza o stacji, nie stan tylko na czas jednej trasy) —
resetuje się dopiero przy ponownym imporcie bazy albo ręcznym odznaczeniu.

**„Wygeneruj e-mail do audytora"** otwiera okno z gotową, czytelną treścią
wiadomości (baza, dni, stacje z adresami, czasy dojazdu i audytu, powrót)
i przyciskiem „Kopiuj do schowka" — wklejasz od razu w e-mail albo komunikator.

**Filtr audytora w nagłówku.** Obok listy „wybierz trasę" jest drugi selektor
...
gdy masz wielu audytorów i szukasz konkretnej trasy bez przewijania pełnej
...
| `MAX_STATIONS_PER_AUDITOR` | wstępny limit stacji na audytora (przed podziałem na bazy) | 80 |
...
**Brak wspólnych tras dla kilku osób.** Funkcja pozwalająca dwóm audytorom
...
oddaniem aplikacji do użytku. Każdy audytor planuje obecnie osobną trasę.
...
„Wygeneruj e-mail do audytora" opisuje dni jako „Dzień 1", „Dzień 2" itd. —
