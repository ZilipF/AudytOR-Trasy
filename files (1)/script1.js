...
(function(){
  ...
  var CONFIG = {
    ...
    // Twardy limit serwera OSRM: jedno zapytanie o macierz może objąć
    // maksymalnie 100 punktów (100 × 100 = 10 000 wartości). Przekroczenie
    // kończy się błędem, dlatego żadne zadanie nie może mieć więcej niż
    // MAX_MATRIX_POINTS - 1 stacji (jeden punkt zajmuje baza).
    MAX_MATRIX_POINTS: 100,
  ...
  function fetchDistanceMatrix(points){
    if(points.length > CONFIG.MAX_MATRIX_POINTS){
      return Promise.reject(new Error(
        'Za dużo punktów w jednym zapytaniu (' + points.length + ', limit ' +
        CONFIG.MAX_MATRIX_POINTS + '). Zmniejsz liczbę stacji albo dodaj audytora.'
      ));
    }
    ...
    return fetch(url)
      .then(function(response){
        if(!response.ok){
          throw new Error('Serwer tras odrzucił zapytanie (' + response.status +
            ') dla ' + points.length + ' punktów.');
        }
        return response.json();
      })
      .then(function(data){
  ...
  function assignStationsToAuditors(stations, auditors){
    ...
    var maxPerAssignment = Math.min(
      CONFIG.MAX_STATIONS_PER_AUDITOR,
      CONFIG.MAX_MATRIX_POINTS - 1
    );
    ...
    byBase.forEach(function(group){
      ...
      // Stacje najpilniejsze mają pierwszeństwo, gdy trzeba coś odciąć.
      var byRisk = group.stations.slice().sort(function(a, b){
        return (b.risk || 0) - (a.risk || 0);
      });

      var groupCap = group.auditors.length * maxPerAssignment;
      if(byRisk.length > groupCap){
        skipped += byRisk.length - groupCap;
        byRisk = byRisk.slice(0, groupCap);
      }

      var clusters = RouteEngine.clusterStations(byRisk, group.auditors.length);
      clusters.forEach(function(cluster, idx){
        var auditor = group.auditors[idx];
        if(!auditor || !cluster.length){ return; }

        // Klastrowanie wyrównuje grupy, ale nie gwarantuje limitu — przy
        // jednym audytorze na bazę cały jej ruch trafiłby do jednego zadania.
        var stations = cluster;
        if(stations.length > maxPerAssignment){
          stations = stations.slice().sort(function(a, b){
            return (b.risk || 0) - (a.risk || 0);
          }).slice(0, maxPerAssignment);
          skipped += cluster.length - stations.length;
        }
  ...
  function handleBuildPlan(){
    ...
    var assignment = assignStationsToAuditors(capped, auditors);
    ...
    if(assignment.skipped > 0){
      capNote = ' Uwaga: pominięto ' + skipped + ' stacji o najniższym priorytecie ' +
        '(na jednego audytora przypada maks. ' +
        Math.min(CONFIG.MAX_STATIONS_PER_AUDITOR, CONFIG.MAX_MATRIX_POINTS - 1) + ' stacji).';
      console.warn('Pominięto', skipped, 'stacji ponad limit planowania.');
    }
