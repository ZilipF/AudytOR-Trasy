...
(function(global){
   ...
   */
  /**
   * @param days ile dni ZAŻĄDANO (np. wariant "3-dniowy" z UI).
   *
   * Nie zawsze próbuje od razu wszystkich `days` dni — zaczyna od najmniejszej
   * sensownej liczby (tyle, ile jest stacji, każda mogłaby dostać osobny dzień
   * w najgorszym razie) i zwiększa TYLKO wtedy, gdy to za mało, żeby zmieścić
   * się w limitach czasu. To rozróżnia dwie różne sytuacje, które łatwo
   * pomylić:
   *
   *  - Za mało STACJI na zażądaną liczbę dni (np. 2 stacje przy wariancie
   *    3-dniowym) — dodatkowy dzień byłby tylko pustą, doklejoną wydmuszką,
   *    identyczną z krótszym wariantem. Nie dokładamy go.
   *  - Za mało CZASU na mniejszą liczbę dni, mimo małej liczby stacji (jedna
   *    bardzo odległa stacja, gdzie sam dojazd i powrót nie mieszczą się
   *    w jednym dniu) — tu dodatkowy dzień jest naprawdę potrzebny, nie
   *    kosmetyczny, więc próbujemy go dołożyć, zanim uznamy wariant za
   *    niewykonalny.
   */
  function evaluateSchedule(orderedIdx, durMatrix, auditMin, days){
    ...
    var minDays = Math.min(days, stationCount);

    for(var effectiveDays = minDays; effectiveDays <= days; effectiveDays++){
      var result = evaluateScheduleForDays(orderedIdx, durMatrix, auditMin, effectiveDays, stationCount);
      if(result){ return result; }
    }
    return null;
  }

  function evaluateScheduleForDays(orderedIdx, durMatrix, auditMin, effectiveDays, stationCount){
    var maxTotalMin = effectiveDays * BASE_DAY_MIN;
    ...
    // Dopóki effectiveDays <= stationCount, każdy dzień MUSI mieć stację —
    // pusty byłby zawsze niepotrzebną wydmuszką. Dopiero gdy effectiveDays
    // przekracza liczbę stacji (bo mniejsza liczba dni okazała się
    // niewykonalna czasowo — patrz evaluateSchedule wyżej), pusty dzień jest
    // dopuszczalny: to oznacza, że tego dnia jest tylko długi powrót/dojazd,
    // bez własnego przystanku, i naprawdę potrzebuje osobnego dnia.
    var allowEmptyDay = effectiveDays > stationCount;
    ...
    function inspectSplit(cuts){
      ...
      for(var day = 0; day < effectiveDays; day++){
        ...
        if(end === start && !allowEmptyDay){ return; }

        var active = prefix[end] - prefix[start];
        if(day === effectiveDays - 1){ active += returnMin; }
      ...
      // Spośród podziałów mieszczących się w limitach wybieramy najbardziej
      // WYRÓWNANY (najmniejsze odchylenie standardowe obciążenia dni) —
      // realistyczny, czytelny plan zamiast jednego dnia "na styk" i drugiego
      // ledwie ruszonego. totalMin dominuje wynik (nadal liczy się krótsza
      // trasa), wyrównanie rozstrzyga między wariantami o podobnym czasie.
      var avg = totalMin / effectiveDays;
      var variance = 0;
      for(var d = 0; d < dayTotals.length; d++){
        variance += (dayTotals[d] - avg) * (dayTotals[d] - avg);
      }
      var spread = Math.sqrt(variance / effectiveDays);
      var score = totalMin + spread * 3;
    ...
    var breakdown = [];
    var segStart = 0;
    for(var day2 = 0; day2 < effectiveDays; day2++){
   ...
   */
  /**
   * @param candidates lista { station, idx, mandatory? } — mandatory=true
   *   oznacza stację, którą trasa MUSI zawierać (patrz "Uzupełnij trasę"
   *   w aplikacji: ręcznie wybrane stacje). Bez tego zwykły dobór po wadze
   *   (priority = station.risk) nigdy by ich nie wybrał, skoro ręcznie
   *   wybiera się zwykle właśnie stacje BEZ wagi — inaczej i tak wpadłyby
   *   do automatu. Obowiązkowe stacje stają się jedynym, ustalonym punktem
   *   startowym (zamiast kilku niezależnych prób od najwyższej wagi) —
   *   reszta trasy dobierana jest wokół nich dokładnie tak samo jak zawsze.
   */
  function constructFeasibleRoute(candidates, durMatrix, distMatrix, auditMin, days){
    ...
    var mandatory = eligible.filter(function(c){ return c.mandatory; });

    var seedCandidates = mandatory.length ? [null] : eligible.slice().sort(function(a, b){
    ...
    seedCandidates.forEach(function(seed){
      var order, schedule, used;

      if(mandatory.length){
        // Kolejność startowa wśród obowiązkowych: najbliższy sąsiad od bazy,
        // potem od ostatnio dodanej — tylko żeby dać 2-opt niżej rozsądny
        // punkt wyjścia zamiast przypadkowej kolejności, nie ostateczna.
        order = [0];
        var remaining = mandatory.slice();
        while(remaining.length){
          var lastIdx = order[order.length - 1];
          remaining.sort(function(a, b){
            return durMatrix[lastIdx][a.idx] - durMatrix[lastIdx][b.idx];
          });
          order.push(remaining.shift().idx);
        }

        schedule = evaluateSchedule(order, durMatrix, auditMin, days);
        // Same obowiązkowe stacje już się nie mieszczą w limitach — twarda
        // niewykonalność, nie ma czego dalej próbować dobierać wokół nich.
        if(!schedule){ return; }

        used = {};
        order.slice(1).forEach(function(idx){ used[idx] = true; });
      } else {
        order = [0, seed.idx];
        schedule = evaluateSchedule(order, durMatrix, auditMin, days);
        if(!schedule){ return; }
  ...
  function buildRouteForCluster(result, days, auditMin){
    ...
    return {
      ...
      // Realna liczba dni z harmonogramu, nie ta zażądana — mogą się różnić,
      // gdy stacji jest mniej niż zażądanych dni (patrz evaluateSchedule,
      // effectiveDays). Bez tego np. 2 stacje przy zażądanych 3 dniach
      // i tak zgłaszałyby się jako "trasa 3-dniowa" z pustym dniem 3.
      days: schedule.dayTotals.length,
