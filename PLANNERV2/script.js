...
(function(){
  ...
  function fetchDistanceMatrix(points){
    function attempt(){
      ...
      return fetch(url)
        .then(function(response){
          if(!response.ok){ throw new Error('Błąd OSRM (HTTP ' + response.status + ')'); }
          return response.json();
        })
        .then(function(data){
          if(data.code !== 'Ok'){
  ...
  function handleBuildPlan(){
    ...
    runSequentially(assignments, prepareAssignment, CONFIG.OSRM_REQUEST_DELAY_MS)
      .then(function(prepared){
        ...
        // propozycji — te dotyczyły innego zestawu wolnych stacji.
        lastPlans = plans;
        lastPlanRouteIds = [];
  ...
  function buildProposalCard(plan, isBest){
    ...
    // Rozbicie na audytorów ma sens tylko przy więcej niż jednym — przy
    // jednym powielałoby dokładnie to, co już widać w statystykach wyżej.
    var chipsHtml = '';
    if(plan.routes.length > 1){
      var chips = plan.routes.map(function(route){
        return '<span class="day-chip">' + escapeHtml(route.auditorName) + ': ' +
          fmtCount(route.orderedStations.length, ['stacja', 'stacje', 'stacji']) +
          ', ' + fmtMin(route.totalMin) + '</span>';
      }).join('');
      chipsHtml = '<div class="day-chips">' + chips + '</div>';
    }

    card.innerHTML =
      '<div class="proposal-head">' +
        '<h3>Plan zespołowy · ' + plan.days + ' ' + dayLabel + '</h3>' +
        '<span>' + fmtCount(plan.routes.length, ['trasa', 'trasy', 'tras']) + '</span>' +
      '</div>' +
  ...
  function commitTeamPlan(plan){
    ...
    var toFetch = plan.routes.map(function(planned){
    ...
    runSequentially(toFetch, function(item){
      return fetchRouteGeometry(item.points).then(function(osrmRoute){
        ...
        if(roundtrip){ item.route.totalDist = osrmRoute.distance; }
      });
  ...
  function renderMap(){
    ...
    // pinezki, kolorowane wg grupy (CODO/DOFO) — patrz legenda nad mapą.
    if(!route){
      state.stations.forEach(function(station){
        markersLayer.addLayer(
          L.marker([station.lat, station.lng], { icon: pinIcon('', stationGroupClass(station)) })
    ...
    stations.forEach(function(station, idx){
      ...
      markersLayer.addLayer(
        L.marker([station.lat, station.lng], {
          icon: pinIcon(String(idx + 1), idx === 0 ? 'start' : stationGroupClass(station))
        }).bindPopup(stationPopup(station))
      );
    });
    ...
    // pinezki — podwójne kliknięcie dopisuje stację na koniec trasy.
    state.stations.forEach(function(station){
      if(station.status !== 'free' || inRoute[station.id]){ return; }

      var ghost = L.marker([station.lat, station.lng], {
        icon: pinIcon('', 'ghost ' + stationGroupClass(station)),
